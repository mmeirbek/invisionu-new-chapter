    ...
    <article className="flex flex-col gap-3 rounded-panel border border-border-subtle bg-bg-surface p-5">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-[0.8rem] text-text-muted">{looksFor}</p>
          </div>
        </div>
      ...
      {supported ? (
        ...
        <p className="rounded-control bg-bg-elevated px-3 py-2 text-[0.8rem] text-text-secondary">
          There is not enough verified evidence to score this. It is a question for the live interview, not a low mark.
        </p>
      )}
    </article>
  );

...
| **M5 · Bias Guard** | Leading questions, uneven coverage, an interviewer's drift on the scale | Commission |

The candidate gets developmental feedback after the simulation: no scores, no ranking, no hint of a decision.

## Principles

- Every AI claim carries evidence — a verbatim quote and its source. Without evidence the answer is `null`, never a low score.
...
- Integration is JSON in, JSON out. Scenarios, rubric and model choices live in configuration, not in code.

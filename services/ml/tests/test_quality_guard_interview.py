...
from services.ml.app.modules.quality_guard import InterviewQuestionAnalyzer, ProposedSignal, QuestionProposal
from services.ml.app.schemas.contracts import Evidence, QualityCheckRequest
...
def leading(source_id: str = "iturn_03", quote: str | None = None) -> ProposedSignal:
    return ProposedSignal(
        kind="leading_question", message="The question suggests its own answer.",
        recommendation="Ask openly about what mattered and why.", competencies=[],
...
def test_planted_signals_and_model_boundary() -> None:
    proposed = QuestionProposal(signals=[
        ...
        ProposedSignal(
            ...
            recommendation="Remove the question and return to the rubric.", competencies=[],
            evidence=[Evidence(source="interview_turn", sourceId="iturn_07",
                               quote="What do your parents do for a living?")],
        ),
        ProposedSignal(kind="coverage_gap", message="Vision and Values were not explored.",
                       recommendation="Ask open questions about I and V.", competencies=["I", "V"], evidence=[]),
    ])
